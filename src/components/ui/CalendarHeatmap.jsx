        ...
        {weeks.map((week, weekIndex) => (
          <div key={weekIndex} className="flex gap-0.5 justify-center">
            {week.days.map((day, dayIndex) => (
              ...
              >
                <div
                  className={`w-3 h-3 rounded-sm transition-colors ${getIntensityColor(day.hasActivity, day.intensity)}`}
  ...
  return (
    <div className="flex gap-1 justify-center">
      {days.map((day, index) => (
        ...
        >
          <div
            className={`w-8 h-8 rounded-md transition-colors ${getIntensityColor(day.hasActivity, day.intensity)}`}
            title={`${day.date}: ${day.hasActivity ? `Swam ${(day.totalDistance / 1000).toFixed(1)}km` : 'Rest day'}`}
          />
